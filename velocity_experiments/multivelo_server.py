...
total_counts_path = "/omics/groups/OE0132/tandem/nschmidt/MultiVelo_Data/all_rna_counts.h5ad"  # originally _normalized
...
adata_atac = mv.aggregate_peaks_10x(adata_atac_intersected, str(peak_annotation), str(feature_linkage))

shared_cells = pd.Index(np.intersect1d(adata_rna.obs_names, adata_atac.obs_names))
shared_genes = pd.Index(np.intersect1d(adata_rna.var_names, adata_atac.var_names))

#### Reload raw data and subset ####
adata_rna = sc.read_h5ad(str(total_counts_path))
# scv.pp.filter_and_normalize(adata_rna, min_shared_counts=10, n_top_genes=1000)  # Filtering can be skipped, as this is done by using the shared_genes! 
adata_rna.obs.rename(columns={"cell_type": "celltype"}, inplace=True)
...
multivelo_results_path = f"{output_path}multivelo_results_v2.h5ad"
